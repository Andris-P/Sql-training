...
const insertMovieGenres = (
  ...
  const values = genres.map((genre) => {
    ...
    return genreRow ? `(${movieId}, ${genreRow.id})` : null;
  }).join(", ");
...
const insertMovieActors = (
  ...
  const values = actors.map((full_name) => {
    const actorRow = actorRows.find((row) => row.full_name === full_name);
    return actorRow ? `(${movieId}, ${actorRow.id})` : null;
  }).join(", ");
...
const insertMovieDirectors = (
  ...
  const values = directors.map((full_name) => {
    const directorRow = directorRows.find((row) => row.full_name === full_name);
    return directorRow ? `(${movieId}, ${directorRow.id})` : null;
  }).join(", ");
...
const insertMovieKeywords = (
  ...
  const values = keywords.map((keyword) => {
    ...
    return keywordRow ? `(${movieId}, ${keywordRow.id})` : null;
  }).join(", ");
...
const insertMovieProductionCompanies = (
  ...
  const values = productionCompanies.map((company_name) => {
    const production_companieRow = productionCompanyRows.find((row) => row.company_name === company_name);
    return production_companieRow ? `(${movieId}, ${production_companieRow.id})` : null;
  }).join(", ");
