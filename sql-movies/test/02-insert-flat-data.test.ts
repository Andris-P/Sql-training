...
const insertActors = (_actors: string[]) => {
  return (
    `insert into actors (full_name) values` +
    _actors.map(actor => `('${escape(actor)}')`).join(",")
  );
};

const insertKeywords = (_keywords: string[]) => {
  return `INSERT INTO keywords (keyword) VALUES` +
      _keywords.map(keyword => `('${escape(keyword)}')`).join(",");
};

 const insertDirectors = (_directors: string[]) => {
  return `INSERT INTO directors (full_name) VALUES` +
  _directors.map(director => `('${escape(director)}')`).join(",");
 };

 const insertGenres = (_genres: string[]) => {
  return `INSERT INTO genres (genre) VALUES`+ 
      _genres.map(genre => `('${escape(genre)}')`).join(",");
 }; 

const insertProductionCompanies = (_companies: string[]) => {
  return `INSERT INTO production_companies (company_name) VALUES`+ 
  _companies.map(company => `('${escape(company)}')`).join(",");
};


const insertMovies = (_movies: Movie[]) => {
  const values = _movies.map(movie => {
    const {
      imdbId, popularity, budget, budgetAdjusted, revenue, revenueAdjusted,
      originalTitle, homepage, tagline, overview, runtime, releaseDate
    } = movie;
    const taglineValue = tagline ? `'${escape(tagline)}'` : 'NULL';
    const homepageValue = homepage ? `'${escape(homepage)}'` : 'NULL';
    return `('${escape(imdbId)}', ${popularity}, ${budget}, ${budgetAdjusted}, ${revenue}, ${revenueAdjusted},
     '${escape(originalTitle)}', ${homepageValue}, ${taglineValue}, '${escape(overview)}',
      ${runtime}, '${escape(releaseDate)}')`;
  }).join(",");
  return `INSERT INTO movies (imdb_id, popularity, budget, budget_adjusted, revenue, revenue_adjusted,
     original_title, homepage, tagline, overview, runtime, release_date) VALUES ${values}`;
};
